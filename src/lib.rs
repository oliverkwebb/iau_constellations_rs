//! # Fast IAU Constellation Algorithm In Rust
//!
//! The implementation of this is highly optimized, determining the constellation without floating-point math.
//!
//! The code itself is a fork of <https://github.com/Bill-Gray/lunar/blob/master/conbound.c>
//!
//! ```
...
/// The names of the 88 IAU constellations sorted alphabetically
pub const CONSTELLATION_NAMES: [&str; 88] = [
    "Andromeda",
    "Antlia",
    "Apus",
    "Aquarius",
...
fn spd_of(i: isize) -> i32 {
    BOUNDS[i as usize].0 >> 17
}

fn ra_of(i: isize) -> i32 {
    BOUNDS[i as usize].0 & 0x1ffff
}

/// Gets the constellation with B1875.0 equatorial coordinates in degrees.
///
/// Returns an index of the constellation names sorted alphabetically, can be used to index `CONSTELLATION_NAMES`
pub fn constell_1875(ra_degrees_1875: f64, dec_degrees_1875: f64) -> usize {
    let ra: i32 = (((ra_degrees_1875 * 240.0) + 864000.0) % 86400.0) as i32;
    let spd: i16 = ((dec_degrees_1875 + 90.0) * 60.0) as i16;
    let mut idx: isize = -1;
    let mut step = 512;
    let mut rval: i8 = -1;
    let n_bounds: usize = BOUNDS.len();

    while step != 0 {
        if idx + step < n_bounds as isize && (spd as i32) < spd_of(idx + step as isize) {
            idx += step;
        }
    ...
    while idx >= 0 && rval == -1 {
        let min_ra = ra_of(idx);
        let max_ra = min_ra + (BOUNDS[idx as usize].1 as i32);

        if (ra >= min_ra && ra < max_ra) || (ra + 86400 >= min_ra && ra + 86400 < max_ra) {
            rval = BOUNDS[idx as usize].2 as i8;
        }
        idx -= 1;
    }
    if rval == -1 {
        /* Didn't hit anything;  must be in UMi */
        rval = 83;
    }
    rval as usize
}
